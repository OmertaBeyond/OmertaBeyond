...
function setArr(num){ return arr[num] = "<a href=/user.php?nick=" + arr[num].match(/\w+/g)[0] + "><b>" + arr[num] +"</b></a>"; }
...
function getActual(el,attr) { return document.defaultView.getComputedStyle(el, '').getPropertyValue(attr); };
function selectedTab() { return $X('//a[contains(@class, "selected")]/@href').nodeValue; }
...
function refreshIn(){
	...
	setTimeout("window.location.reload()", addExtraSec);
}
...
function checkNRdiv(url) {
	on = (document.getElementById('alt').textContent == '1' || document.getElementById('ctrl').textContent == '1')?1:0;
	go = 1;
	if(document.body.innerHTML.indexOf('id="'+url) != -1) { //check for div
		popup = document.getElementById(url);
		if(on) popup.style.display = 'block'; 
		go = 0;
	...
	if(go && on) {
		div = document.createElement('div');//add popup
		div.id = url;
		...
		document.addEventListener('mousemove', function(mouse) {
			divH = div.scrollHeight;
			divW = div.scrollWidth;
			X = mouse.pageX;
			Y = mouse.pageY;
			plusX = 20;
			plusY = 20;
