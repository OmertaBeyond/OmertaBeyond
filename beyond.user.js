...
// --customs script
	...
	if (!dls || dls.indexOf('reset') != -1 || dls.indexOf('buttons') != -1){// mode = normal or user reseted
		...
		$X(QL_xp).setAttribute('colspan', '2');// adept quick lookup colspan to match customs interface's
	}
...
if (prefs[2] && dlp == '/info.php'){
	function addNews(){
		...
		for(i=0;i<oNews.length;i++){// add to page
			...
			item.href = news[i][0];
			item.setAttribute('style', 'font-size:11px !important;');// manually override to make sure it keeps font-size
...
if (dlp == '/prices.php' || dlp == '/smuggling.php' || dlp == '/travel.php') {
	...
	function appBRC(BN) { // Best Run Calculator
		...
		for (allProfits = [], bestBN = [], i = 0; i <= 7; i++) {
			...
			// --Calc profits
			if (i == city - 4) { //T his is the current city
	...
	if (prefs[21] && pp) {
		...
		if (typeof BN == 'undefined') { //s ee if prices are grabbed already
