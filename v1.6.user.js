...
// @description    Omerta Beyond 1.6 (The greatest addon for Omerta available!)
...
var maxbit = 22; //set the amount of preferences
...
if(lh == 'http://www.omertabeyond.com/beyond/prefs.html' + ls ) {
	...
	function go() {
		...
		var prefstr = [
			...
			"Cars, crimes and booze/narcs form",//10
		...
		string += '<td style="text-align: left; margin-left: 3px;">To edit which hotkeys you want to use with Alt+Shit+<i>"letter"</i></td>';
...
//---------------- Menu and submenus ----------------
if(dlp == '/menu.php') {
	if(GM_getValue('mpref')) { //check for preferences
		var mpref = GM_getValue('mpref'); //assembling preferences
		var buttonpref = [GM_getValue('buttonpref1'),GM_getValue('buttonpref2'),GM_getValue('buttonpref3'),GM_getValue('buttonpref4'),GM_getValue('buttonpref5'),GM_getValue('buttonpref6'),GM_getValue('buttonpref7'),GM_getValue('buttonpref8'),GM_getValue('buttonpref9')];
		var keypref = [GM_getValue('keypref1'),GM_getValue('keypref2'),GM_getValue('keypref3'),GM_getValue('keypref4'),GM_getValue('keypref5'),GM_getValue('keypref6'),GM_getValue('keypref7'),GM_getValue('keypref8'),GM_getValue('keypref9')];
		var menunames = ["elp","sms","game","com","crime","fam","cas","gh","oth"];
		var num = 1; //which-menu-am-I-at variable
		
		var result = document.evaluate("//div[@id='fam']/table/tbody", document, null, 7, null); //var for comparing fampos in prefs and fampos in menu
		var fambuttons = result.snapshotItem(0); 
		var fambuttons = fambuttons.getElementsByTagName('tr').length;
		for(k=0;k<=mpref.length-1;k++){ //loop main menu preferences
			if(k==1) { //compensate for irregular sms menu
				if(mpref.slice(1,2) == "0") { del("//a[@id='xsms']"); delall("//div[@id='sms']"); }
				else { 
					var j = 1;//xpath number to select right line in menu
					for(i=0;i<=3;i++) { //loop sms menu prefs
						h=i+1;
						if(buttonpref[k].slice(i,h) == "0") { del("//div[@id='sms']/table/tbody/tr["+j+"]") }
						else { //add hotkeys
							if(GM_getValue('keypref1')) { if(keypref[k].slice(i,h) != "*") { 
								xpath("//div[@id='sms']/table/tbody/tr["+j+"]/td/a").snapshotItem(0).accessKey = keypref[k].slice(i,h);
								xpath("//div[@id='sms']/table/tbody/tr["+j+"]/td/a").snapshotItem(0).innerHTML = xpath("//div[@id='sms']/table/tbody/tr["+j+"]/td/a").snapshotItem(0).innerHTML + " (" + keypref[k].slice(i,h) + ")"; }
							}
							var j = j + 1; //compensate for non removed button (select next button)
						}
					}
					var num = num + 0;
				}
				if(mpref.slice(0,1) != "0") {var num = num + 1;} //compensate for non removed menu (select next menu)
			}
			else {
				if(k==mpref.length-1) { del("//div[@id='oth']//tr[9]//font"); del("//div[@id='oth']//tr[9]//hr"); del("//div[@id='oth']//tr[9]//br") }
				if(mpref.slice(k,k+1) == "0") { del("/html/body/table["+num+"]"); delall("//div[@id='"+menunames[k]+"']"); }
				else { 
					var j = 1;//xpath number to select right line in menu
					for(i=0;i<=buttonpref[k].length-1;i++) { //loop other menu prefs
						h=i+1;
						if(buttonpref[k].slice(i,h) == "0") { del("//div[@id='"+menunames[k]+"']/table/tbody/tr["+j+"]") }
						else { //add hotkeys + (hotkey) reminder
							if(GM_getValue('keypref1')) { 
								if(keypref[k].slice(i,h) != "*") { 
									if(k==5) { //family menu
										if(i<fambuttons) { //check saved fampos with current fampos in the menu
											xpath("//div[@id='"+menunames[k]+"']/table/tbody/tr["+j+"]/td/a").snapshotItem(0).accessKey = keypref[k].slice(i,h); 
											xpath("//div[@id='"+menunames[k]+"']/table/tbody/tr["+j+"]/td/a").snapshotItem(0).innerHTML = xpath("//div[@id='"+menunames[k]+"']/table/tbody/tr["+j+"]/td/a").snapshotItem(0).innerHTML + " (" + keypref[k].slice(i,h) + ")";
										}
									}
									else { 
										xpath("//div[@id='"+menunames[k]+"']/table/tbody/tr["+j+"]/td/a").snapshotItem(0).accessKey = keypref[k].slice(i,h);
					...
					if(k!=0) { var num = num + 1;} //compensate for non removed menu (select next menu)
				}
			}
		}
	}
	var body = document.body; //change alluser url
	body.innerHTML = body.innerHTML.replace('allusers.php', 'allusers.php?start=0&order=lastrank&sort=DESC');
	...
	if(prefs[0]) { //If Beyond Menu preference is on
		var starter = CreateTitle("Beyond", "beyond");
		var menu = CreateMenu("beyond");
		
		var xgame = getID('xgame');
		xgame.parentNode.insertBefore(starter, xgame);
		xgame.parentNode.insertBefore(menu, xgame);
		
		AddRow("beyond", "http://www.omertabeyond.com/beyond/prefs.html", "Preferences", true);
		for(j=0; j<querys.length && querys[j].indexOf("menu_") != -1; j++) {
			if(GM_getValue(querys[j], '0') == '1') AddRow("beyond", qlinks[j], descr[j]);
		}
	} 
	else {
		var menu = CreateRow("http://www.omertabeyond.com/beyond/prefs.html", "Preferences", true);
		var game = getID('game');
		game.parentNode.insertBefore(menu, game);
	}
	if(prefs[3]) { //If Calculator Menu preference is on
		...
		var l = 'http://www.omertabeyond.com/old/calculators/';
		AddRow("calc", l+"oc.html", "OC Calculator");
		AddRow("calc", l+"moc.html", "MOC Calculator");
		AddRow("calc", l+"sh.html", "SH Calculator");
		AddRow("calc", l+"bank.html", "Bank Calculator");
	}
}
...
if(dlp == '/jail.php') {
	var x = document.evaluate("/html/body", document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
	var x = x.snapshotItem(0).innerHTML;
	if (x.search(/table/i) != -1) {
			...
			if(getTAG('input')[6] != null) {
				if (prefs[5]) { 
					window.addEventListener("load", function() { 
						 getTAG('input')[5].checked = true;
						 getTAG('input')[6].focus();
					}, true); 
...
var wrongcode = "<br><center>Learn to type, analphabetic!<br><br><a href=\"javascript:history.back()\">Click here to go back or wait a second</a></center>";
if(dlp + dls == '/BeO/webroot/index.php?module=Crimes&action=docrime' && prefs[13]) {
    var results = document.evaluate("//html/body", document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
...
if((dlp + dls) == ('/user.php' + dls)){
	...
	if(prefs[19]) { //if detailed w/r is on
		...
		var results = document.evaluate(xpath, document, null, 6, null);
		var rf = results.snapshotItem(0).innerHTML;
		...
		for(i=0;i<=10;i++) {
			var y = x[i];
			var i = i + 1;
			if(rf.indexOf(y) == 0) { results.snapshotItem(0).innerHTML = i + " - " + results.snapshotItem(0).innerHTML; }
			var i = i - 1;
		}
	}
...
if(dlp + dls == '/compress.php?r=statistics') {
	...
	if(prefs[15]) {//if add quicklinks is on
		var result = document.evaluate("/html/body/center", document, null, 6, null);
		result.snapshotItem(0).innerHTML = "<table class='thinline' width='600' rules='none' cellspacing='0' cellpadding='2'><tbody><tr><td class='tableheader' align='center' colspan='4'><a href='#dfams'><b>Death Fams</b></a> - <a href='#honour'><b>Honoured</b></a> - <a href='#cdtc'><b>CDTC</b></a> - <a href='#fams'><b>Families</b></a> - <a href='#bf'><b>BF</b></a> - <a href='#book'><b>Bookie</b></a> - <a href='#roul'><b>Roullie</b></a> - <a href='#num'><b>Numbers</b></a> - <a href='#slot'><b>Slots</b></a> - <a href='#bj'><b>BJ</b></a> - <a href='#pb'><b>PB</b></a></td></tr><tr></tr><td colspan='3' bgcolor='black' height='1'></td></tr></tbody></table>" + result.snapshotItem(0).innerHTML;
		...
		for(i=0;i<=10;i++) {
			j=i+5;
			x[i]=a+j+b;
			var result = document.evaluate(x[i], document, null, 6, null);
			result.snapshotItem(0).innerHTML = "<a name='" + y[i] + "'>"  + result.snapshotItem(0).innerHTML + "</a>";
		}
	}
}
...
if((dlp + dls) == ('/family.php' + dls)){
	if(prefs[16]) {
		...
		var result = document.evaluate(xpath, document, null, 6, null).snapshotItem(0).innerHTML;
...
if(dlp == '/information.php') { //grab missing blood amount
	if(prefs[10]){
		...
		var healthres = document.evaluate(healthxpath, document, null, 7, null)
		var missingHealth = '100' - healthres.snapshotItem(0).textContent.replace('%','')
		GM_setValue('health',missingHealth);
		if(dlp == '/bloodbank.php'){ //fill the missing blood into box
			if(GM_getValue('health') != 0 ) {
				getTAG('input')[0].value = GM_getValue('health');
			}
		}
	}
}
...
if(lh == 'http://www.omertabeyond.com/gm/menuprefs.htm' + ls) {
	if(ls.length>1 && ls.indexOf("game") != -1) { //update prefs
		...
		GM_setValue('buttonpref1',GetParam('elp'));
		GM_setValue('buttonpref2',GetParam('sms'));
		GM_setValue('buttonpref3',GetParam('game'));
		GM_setValue('buttonpref4',GetParam('com'));
		GM_setValue('buttonpref5',GetParam('crime'));
		GM_setValue('buttonpref6',GetParam('fam'));
		GM_setValue('buttonpref7',GetParam('cas'));
		GM_setValue('buttonpref8',GetParam('gh'));
		GM_setValue('buttonpref9',GetParam('oth'));
...
if(lh == 'http://www.omertabeyond.com/gm/hotkeyprefs.htm' + ls) {
	if(ls.length>1 && ls.indexOf("game") != -1) {//update prefs
		...
		GM_setValue('keypref1',GetParam('elp'));
		GM_setValue('keypref2',GetParam('sms'));
		GM_setValue('keypref3',GetParam('game'));
		GM_setValue('keypref4',GetParam('com'));
		GM_setValue('keypref5',GetParam('crime'));
		GM_setValue('keypref6',GetParam('fam'));
		GM_setValue('keypref7',GetParam('cas'));
		GM_setValue('keypref8',GetParam('gh'));
		GM_setValue('keypref9',GetParam('oth'));
		setTimeout("history.go(-2)",1500)
	}
	if(GM_getValue('keypref1')) { var buttonpref = [GM_getValue('keypref1'),GM_getValue('keypref2'),GM_getValue('keypref3'),GM_getValue('keypref4'),GM_getValue('keypref5'),GM_getValue('keypref6'),GM_getValue('keypref7'),GM_getValue('keypref8'),GM_getValue('keypref9')]; }
	var menunames = ["elp","sms","game","com","crime","fam","cas","gh","oth"];
	var buttons = [GM_getValue('buttonpref1'),GM_getValue('buttonpref2'),GM_getValue('buttonpref3'),GM_getValue('buttonpref4'),GM_getValue('buttonpref5'),GM_getValue('buttonpref6'),GM_getValue('buttonpref7'),GM_getValue('buttonpref8'),GM_getValue('buttonpref9')];
...
//---------------- Title changer ----------------
if((dlp == '/' || dlp == '/index.php') && prefs[11]) document.title = "Omerta (COM)";
...
GM_update('OBUpdate', 'Omerta Beyond', '1.6', 'http://www.omertabeyond.com', 'http://www.omertabeyond.com/version.txt');