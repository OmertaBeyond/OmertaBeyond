...
$Author: ivdbroek85@gmail.com $:  Author of last commit
$Date: 2012-02-14 18:50:17 +0100 (di, 14 feb 2012) $:  Date of last commit
*/
...
if(dlp == '/garage.php'){
	...
	function garageCrusher() {
		...
		$x('//tr')[rows-1].id = 'GarageActions';

		$(document).ready(function(){
			$(body).css({'position':'relative'});
			var $Gfooter = $('<div id=\"GarageFooter\"></div>').html('<table class="thinline" cellspacing=0 cellpadding=2 rules="none" width="100%"><tr>'+$x('//tr')[rows-1].innerHTML+'</tr></table>');
			$('center').append($Gfooter);
			$Gfooter.css({'display':'block','position':'fixed','bottom':'0','left':((($(window).width() - $Gfooter.outerWidth()) / 2) + $(window).scrollLeft() + "px")});
			$(window).scroll(function(){
				var top = $(window).scrollTop();
				var bottom = top + $(window).height();
				var etop = $('#GarageActions').offset().top;
				var ebottom = etop + $('#GarageActions').height();
				if((ebottom <= bottom) && (etop >= top)) { 
					$Gfooter.hide();
				} else {
					$Gfooter.css({'display':'block','position':'fixed','bottom':'0','left':((($(window).width() - $Gfooter.outerWidth()) / 2) + $(window).scrollLeft() + "px")});
				}
