...
// @namespace      http://beyond.dvsoft.org/beyond/
// @identifier     http://beyond.dvsoft.org/beyond/beyond.user.js
// @description    The greatest addon for Omerta available!
// @include        http://beyond.dvsoft.org/gm/prefs.html*
...
//   Our beta test team: Eagle, Mesmendon, Viva and Dareios
...
var maxbit = 12; //set the amount of preferences
...
var qlinks = ['http://beyond.dvsoft.org/omerta.php?families','http://beyond.dvsoft.org/omerta.php?history','http://beyond.dvsoft.org/omerta.php?deaths','http://beyond.dvsoft.org/omerta.php?honour','http://beyond.dvsoft.org/omerta.php?randomquote','http://beyond.dvsoft.org/omerta.php?prices','http://beyond.dvsoft.org/targets/omerta.php','http://89.149.221.178/~fingon/latestpicture.php']; //links for custom beyond menu
...
//---------------- Preference Pane ----------------
if(lh == 'http://beyond.dvsoft.org/gm/prefs.html' + ls) {
	if(ls.length > 1) {
		if(ls.indexOf("=") == -1) {
	...
	var prefstr = ["Clean useless menus","Beyond submenu on the left","Cocaine Prices in the marquee","Fingon's News in the info menu","Calculators Submenu on the left","Jail Highlighter","Remove 'shooting bottle' from crime page","Click bank amount to fill in bank form","Remove Jailbusting skill","Disable Imageshack/Photobucket/XS images on profiles","Bullet Form (amount will be 400)","Always shoot the bottle"];
	var string = '';
...
if(dlp == '/marquee.php') {
	if(prefs[2]) {
		GM_xmlhttpRequest({
			method: 'GET',
			url: 'http://beyond.dvsoft.org/gm/prices.xml.php',
			...
			onload: function(responseDetails) {
				...
				var link = cEL('a');
				link.href = "http://beyond.dvsoft.org/omerta.php?prices";
				link.target = 'main';
...
if(dlp == '/menu.php') {
	...
	if(prefs[1]) { //If Beyond Menu preference is on
		...
		AddRow("beyond", "http://beyond.dvsoft.org/gm/prefs.html", "Preferences", true);
		for(j=0; j<querys.length && querys[j].indexOf("menu_") != -1; j++) {
			if(GM_getValue(querys[j], '0') == '1') AddRow("beyond", qlinks[j], descr[j]);
		}
	} 
	else {
		var menu = CreateRow("http://beyond.dvsoft.org/gm/prefs.html", "Preferences", true);
		var game = getID('game');
	...
	if(prefs[4]) { //If Calculator Menu preference is on
		...
		AddRow("calc", "http://beyond.dvsoft.org/old/calculators/oc.html", "OC Calculator");
		AddRow("calc", "http://beyond.dvsoft.org/old/calculators/moc.html", "MOC Calculator");
		AddRow("calc", "http://beyond.dvsoft.org/old/calculators/sh.html", "SH Calculator");
		AddRow("calc", "http://beyond.dvsoft.org/old/calculators/bank.html", "Bank Calculator");
	}
	if(prefs[0]) { //If Clean Menu preference is on

        del("//div[@id='elp']//tr[3]//a"); // Omerta Shop
...
if(dlp == '/info.php') {
	if(prefs[3]) { //If Fingon's News menu preference is on
		...
		var url = 'http://89.149.221.178/~fingon/beyond-news.php?num=' + GM_getValue('newsamount', 5) + '&version=1&css='+ menu +'&url=' + location.hostname;
...
if(dlp == '/jail.php') {
    ...
    // set the focus to the input box;
	   if(getTAG('input')[0] != null) { 
	   	  getTAG('input')[2].checked = true;
	      getTAG('input')[0].focus();
	      }
    }
}
...
if(dlp + dls == '/BeO/webroot/index.php?module=Crimes') {
	...
	if(prefs[11]){ //If shoot the bottle is on
...
if(dlp == '/information.php') {
	...
	var results = document.evaluate("/html/body/center/table/tbody/tr/td/table/tbody/tr[6]/td[2]", document, null, 7, null);
	...
	var tcres = document.evaluate("/html/body/center/table/tbody/tr/td/table/tbody/tr[7]/td[2]", document, null, 7, null);
	...
	var res = document.evaluate("/html/body/center/table/tbody/tr/td/table/tbody/tr[4]/td[2]", document, null, 7, null);
